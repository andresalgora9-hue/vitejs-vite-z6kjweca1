      ...
      <button onClick={async()=>{
        try{
          const res=await fetch("https://rjwojxwrsbvwwshwwpvq.supabase.co/functions/v1/super-handler",{
            ...
            headers:{
              "Content-Type":"application/json",
              "apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJqd29qeHdyc2J2d3dzaHd3cHZxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDU0MjA1MzQsImV4cCI6MjA2MDk5NjUzNH0.3aMGMIe7Y3pPPBT7yWwLBpAyMJNyBMFJAf3fNtyO2hI",
              "Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJqd29qeHdyc2J2d3dzaHd3cHZxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDU0MjA1MzQsImV4cCI6MjA2MDk5NjUzNH0.3aMGMIe7Y3pPPBT7yWwLBpAyMJNyBMFJAf3fNtyO2hI",
            },
...
const makeSideCard=(slot:number)=>{
      ...
      {showRegisterStripe&&pendingProFormData&&(
        <StripePayModal
          ...
          onSuccess={async(pl)=>{
            ...
            await db.from("users").update({
              trade:pendingProFormData.trade,
              zone:pendingProFormData.zone,
              plan:pl,
              price:30,
              whatsapp:pendingProFormData.phone,
              free_quote:true,
              service_zones:[pendingProFormData.zone],
            ...
            gtagEvent("purchase",{value:PLAN_PRICES[pl],currency:"EUR",transaction_id:finalUser.id,user_type:"pro",plan:pl});
            gtagEvent("sign_up",{method:"email",user_type:"pro"});
            onLogin(finalUser as UserRow);
          }}
        />
      )}
  ...
  <p style={{fontWeight:800,fontSize:22,color:"#FFD700",margin:0,minHeight:30,
    ...
    transition:"opacity 0.3s ease, transform 0.3s ease"
            ...
            <Btn full disabled={loading} onClick={login}>{loading?"Entrando...":"Entrar →"}</Btn>             <div style={{display:"flex",alignItems:"center",gap:8,margin:"12px 0"}}>               <div style={{flex:1,height:1,background:"rgba(255,255,255,0.08)"}} />               <span style={{fontSize:11,color:C.muted}}>o</span>               <div style={{flex:1,height:1,background:"rgba(255,255,255,0.08)"}} />             </div>             <button onClick={()=>{
  const client=(window as any).google.accounts.oauth2.initTokenClient({
    ...
    callback:async(t:any)=>{
      ...
      const res=await fetch("https://rjwojxwrsbvwwshwwpvq.supabase.co/functions/v1/auth-handler",{
        ...
        body:JSON.stringify({action:"google_auth",email:info.email,name:info.name,avatar_url:info.picture,type:"cliente"})
      });
      const data=await res.json();
      if(data.success){
        if(data.user.type==="profesional"){alert("Para profesionales usa email y contraseña.");return;}
...
}} style={{width:"100%",padding:"13px 16px",background:"#1E2536",border:"1px solid #2D3A52",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",gap:10,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:14,color:"#E8EDF5",boxSizing:"border-box" as const}}>
      ...
      <button onClick={()=>{setShowForgot(false);setForgotMsg("");}}
        ...
        {mode==="register_cliente"&&(
          <GCard style={{background:"rgba(22,27,39,0.82)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderRadius:20,border:"1px solid rgba(255,215,0,0.12)"}}>
            ...
            <button onClick={()=>{
  if(!(window as any).google?.accounts?.oauth2){alert("Google no disponible. Recarga la página.");return;}
  const client=(window as any).google.accounts.oauth2.initTokenClient({
    client_id:"616004854667-tmqmecrr4536qcdsfmfn316n1rlg3lpe.apps.googleusercontent.com",
    scope:"email profile openid",
    callback:async(t:any)=>{
      ...
      if(data.success){
        localStorage.setItem("oy_user",JSON.stringify(data.user));
        window.location.reload();
      }else{setErr(data.error||"Error con Google");}
    }
  });
  client.requestAccessToken();
}} style={{width:"100%",padding:"13px 16px",background:"#1E2536",border:"1px solid #2D3A52",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",gap:10,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:14,color:"#E8EDF5",boxSizing:"border-box" as const,marginBottom:12}}>
  <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.18 1.48-4.97 2.31-8.16 2.31-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg>
  Continuar con Google
</button>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}><div style={{flex:1,height:1,background:C.border}}/><span style={{fontSize:11,color:C.muted,fontWeight:600}}>o con email</span><div style={{flex:1,height:1,background:C.border}}/></div>
            {err&&<div style={{color:C.red,fontSize:13,marginBottom:12,padding:"10px 12px",background:C.red+"15",borderRadius:8,border:"1px solid "+C.red+"33"}}>{err}</div>}

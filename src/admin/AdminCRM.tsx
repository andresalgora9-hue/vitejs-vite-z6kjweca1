...
import {
  DealRow, DealRequestRow, Stage, STAGE_LABEL,
...
// ── PRINCIPAL ───────────────────────────────────────────────
export default function AdminCRM({ sub }: { sub: string }) {
  ...
  // ── FILTRO CIUDAD ──
  const filtra = (ds: DealRow[]) => ciudadF ? ds.filter((d) => d.city_id === ciudadF) : ds;
      ...
      {sub === "cobros" && (
        <>
          ...
          {paraCobrar.map((d) => {
            ...
            return (
              <Caja key={d.id} borde={dias > 21 ? C.red + "55" : C.border}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                  <div style={{ flex: 1, minWidth: 220 }}>
                    <span style={{ color: C.text, fontWeight: 700, fontSize: 14 }}>{d.pro_name}</span>
                    <span style={{ color: C.muted, fontSize: 12 }}> · {d.client_name} · {d.trade}</span>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    <Bt small color={C.green} onClick={() => setMPago(d)}>Liquidar</Bt>
                  </div>
                </div>
              </Caja>
            );
